...
class MyMangaOCR:
    ...
    def __init__(self, language: str = 'en'):
        ...
        if self.language == 'ja':
            try:
                ...
                print("🔤 MangaOCR (японский) готов")
            except ImportError:
                print("⚠️ manga_ocr не установлен. Установите: pip install manga-ocr")
                raise
        else:
            self.reader = easyocr.Reader([self.language], gpu=True)
            self.manga_ocr = None
            print(f"🔤 EasyOCR готов (язык: {self.language})")
    ...
    def _ocr(self, image: np.ndarray) -> Tuple[str, float]:
        if self.reader is not None:
            ...
            text = self.manga_ocr(pil_img).strip()
            return text, 0.9
...
if __name__ == "__main__":
    ...
    ocr = MyMangaOCR(language='ja')
    # blocks = validate_pipeline(detector, PATH_TO_IMAGE, ocr)
    blocks = validate_pipeline(detector, r'./dataset/img jap/The Yandere Pet Cat Is Overly Domineering 3.jpg', ocr)